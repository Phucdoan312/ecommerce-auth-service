        ...
        public AuthResponse login(LoginCommand command) {
                ...
                appClientRepository.findByClientId(command.clientId())
                                .orElseThrow(() -> new AppException(ErrorCode.VALIDATION_ERROR,
                                                "SSO Failed: Ứng dụng [" + command.clientId()
                                                                + "] không tồn tại hoặc chưa đăng ký với hệ thống Auth."));

                User user = userRepository.findByUsernameOrEmail(command.usernameOrEmail(), command.usernameOrEmail())
                                .orElseThrow(() -> new AppException(ErrorCode.INVALID_CREDENTIALS,
                                                "Login Failed: Không tìm thấy User với thông tin đăng nhập: "
                ...
                if (!user.isEnabled()) {
                        throw new AppException(ErrorCode.ACCOUNT_DISABLED,
                                        "Login Failed: Tài khoản [" + user.getUsername()
                                                        + "] đang bị khóa (enabled=false).");
                }

                if (user.getAppRoles() == null || !user.getAppRoles().containsKey(command.clientId())) {
                        log.warn("🚨 [SECURITY ALERT] User {} cố gắng đăng nhập trái phép vào ứng dụng {}",
                                        user.getUsername(), command.clientId());
                        throw new AppException(ErrorCode.VALIDATION_ERROR,
                                        "Access Denied: Tài khoản của bạn không được cấp quyền truy cập vào hệ thống ["
                                                        + command.clientId() + "].");
                }
                ...
                log.info("🔑 [AUDIT] Người dùng [{}] đăng nhập thành công vào hệ thống [{}]", user.getUsername(),
                                command.clientId());
